...
class FcsRuntimeService:
    ...
    def _step_ready_to_advance_locked(self, step: ScheduleStep, now: float) -> tuple[bool, str]:
        ...
        if step.wait_type == "signal":
            ...
            if not ok:
                self._hold_started_monotonic = None
                self._status.hold_elapsed_s = 0.0
                return False, f"Waiting for {step.wait_source} (current={value})"
            return self._evaluate_hold_locked(now, f"Condition met on {step.wait_source}")

        if step.wait_type == "all_valid":
            source_values = {name: self.backend.get_value(name) for name in step.valid_sources}
            all_valid = all(bool(value) for value in source_values.values())
            if not all_valid:
                self._hold_started_monotonic = None
                self._status.hold_elapsed_s = 0.0
                missing = ", ".join(name for name, value in source_values.items() if not bool(value))
                return False, f"Waiting for valid sources: {missing or 'none listed'}"
            return self._evaluate_hold_locked(now, "All valid")

        return False, f"Unknown wait type: {step.wait_type}"

    def _evaluate_hold_locked(self, now: float, label: str) -> tuple[bool, str]:
        if self._hold_started_monotonic is None:
            self._hold_started_monotonic = now
        hold = max(0.0, float(self._active_steps[self._status.current_step_index].hold_for_s or 0.0))
        held = now - float(self._hold_started_monotonic or now)
        self._status.hold_elapsed_s = held
        return held >= hold, f"{label}; hold {held:.1f}/{hold:.1f}s"
    ...
    def _evaluate_signal_condition(self, step: ScheduleStep, value: Any) -> bool:
        ...
        if isinstance(threshold, bool):
            current = bool(value)
            return current == bool(threshold) if op in {"==", ">="} else current != bool(threshold)

        try:
            current_num = float(value)
            threshold_num = float(threshold)
            return {
                ">=": current_num >= threshold_num,
                "<=": current_num <= threshold_num,
                ">": current_num > threshold_num,
                "<": current_num < threshold_num,
                "==": current_num == threshold_num,
            ...
            current_txt = str(value).strip().lower()
            threshold_txt = str(threshold).strip().lower()
            return {
                "==": current_txt == threshold_txt,
                "!=": current_txt != threshold_txt,
            }.get(op, False)
