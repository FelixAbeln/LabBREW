...
def decode_calibration_ack(frame: CanFrame) -> CalibrationAck | None:
    if not isinstance(frame.body, CalibrationAckBody):
        return None
    return CalibrationAck(
        node_id_from_frame(frame),
        frame.body.ack_type,
        frame.body.subindex,
        frame.body.extra,
    )
...
# Calibration command payload is not fully specified in the doc page
# beyond subIndex+bytes,
...
def decode_calibration_cmd(frame: CanFrame) -> CalibrationCmdReceived | None:
