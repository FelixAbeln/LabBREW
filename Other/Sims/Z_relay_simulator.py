    ...
    def serve_forever(self) -> None:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            print(
                ...
                f"(unit_id={self.unit_id}, channels={self.channel_count})"
            )
            print(
                "Commands: status | on <n> | off <n> | toggle <n> "
                "| all on | all off | quit"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fake Modbus TCP relay board for local testing."
    )
    parser.add_argument(
        "--host", default="127.0.0.1", help="Listen host, default: 127.0.0.1"
    )
    parser.add_argument(
        "--port", type=int, default=502, help="Listen port, default: 502"
    )
    parser.add_argument(
        "--unit-id", type=int, default=1, help="Modbus unit/slave id, default: 1"
    )
    parser.add_argument(
        "--channels", type=int, default=8, help="Number of relay channels, default: 8"
    )
