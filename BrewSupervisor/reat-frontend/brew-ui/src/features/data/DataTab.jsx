...
function buildDataRows(rulesSnapshot, dataSearch, starredParams) {
  ...
  const query = String(dataSearch || '').trim().toLowerCase()
...
export function DataRecordingCard({
  dataServiceHealthy,
  isRecording,
  dataServiceStatus,
  dataHz,
...
export function DataLoadstepStatusCard({
  dataServiceHealthy,
  ...
  return (
    <div className="info-card data-card">
      <div className="rules-toolbar">
        <div>
          <h3>Loadstep status</h3>
          <div className="small-text">Latest step capture summary and recording readiness.</div>
        </div>
      ...
      {isTakingLoadstep ? (
        <div className="data-loadstep-status">
          <strong>Loadstep running</strong>
          <span>{loadstepRemainingSeconds != null ? `${loadstepRemainingSeconds}s remaining` : 'Capturing values'}</span>
        </div>
      ) : latestCompletedLoadstep ? (
        <div className="data-loadstep-status">
        ...
        <p className="muted">No completed loadstep captured yet.</p>
      )}
    </div>
  )
}

export function DataSnapshotBrowserCard({
  dataServiceHealthy,
  rulesSnapshot,
  starredParams,
  selectedStarredParams,
  ...
  const dataRows = useMemo(
    () => buildDataRows(rulesSnapshot, dataSearch, starredParams),
    [rulesSnapshot, dataSearch, starredParams],
  )

  return (
    <div className="info-card data-card data-snapshot-card">
      <div className="data-snapshot-top">
        <div className="rules-toolbar">
          <div>
            <h3>Data</h3>
...
export function DataTab(props) {
  return (
    <div className="tab-content-grid data-tab-layout">
      <DataRecordingCard {...props} />
      <DataLoadstepStatusCard {...props} />
      <DataSnapshotBrowserCard {...props} />
    </div>
  )
}
