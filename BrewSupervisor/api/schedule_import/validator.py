...
def _validate_action(
    ...
    *,
    ...
    if kind in {"request_control", "write", "ramp", "release_control"} and action.get(
        "target"
    ):
        _require_parameter(
    ...
    if kind == "global_measurement":
        mode = (
            str(
                action.get("value")
                or (action.get("params") or {}).get("mode")
                or "start"
            )
            .strip()
            .lower()
        ...
        if mode not in {"start", "setup_start", "stop"}:
            _add_issue(
                ...
                path=f"{prefix}.value",
                message=(
                    f"{prefix}: global_measurement mode must be one "
                    "of ['start', 'setup_start', 'stop']"
                ),
                mode=mode,
            )
    ...
    if kind == "take_loadstep":
        ...
        if timing not in valid_timings:
            _add_issue(
                ...
                message=(
                    f"{prefix}: params.timing must be one of "
                    "['on_enter', 'before_next', 'on_exit', 'on_trigger']"
                ),
                timing=timing,
            )
        ...
        if duration_value in (None, ""):
            _add_issue(
                ...
                path=prefix,
                message=(
                    f"{prefix}: duration_s (or params.duration_seconds, "
                    "or measurement_config.loadstep_duration_seconds) "
                    "is required for take_loadstep"
                ),
            )
        else:
            try:
        ...
        if timing == "on_trigger" and trigger_wait is None:
            _add_issue(
                ...
                message=(
                    f"{prefix}: params.trigger_wait is required "
                    "when params.timing is 'on_trigger'"
                ),
            )
        if timing != "on_trigger" and trigger_wait is not None:
            _add_issue(
                ...
                path=f"{prefix}.params.trigger_wait",
                message=(
                    f"{prefix}: params.trigger_wait is only allowed "
                    "when params.timing is 'on_trigger'"
                ),
            )
        if timing == "on_trigger" and trigger_wait is not None:
            if isinstance(trigger_wait, dict):
                ...
                _add_issue(
                    ...
                    path=f"{prefix}.params.trigger_wait",
                    message=(
                        f"{prefix}: params.trigger_wait must be a "
                        "wait expression object"
                    ),
                )
    ...
    if kind == "condition":
        ...
        if cond.get("operator") not in VALID_OPERATORS:
            _add_issue(
                ...
                message=(
                    f"{prefix}.condition: operator must be one of "
    ...
    if loadstep_default not in (None, ""):
        try:
            ...
            _add_issue(
                ...
                message=(
                    "measurement_config.loadstep_duration_seconds "
...
def validate_schedule_payload(
    ...
    *,
    ...
    issues: list[dict[str, Any]] = []
    measurement_cfg = (
        payload.get("measurement_config")
        if isinstance(payload.get("measurement_config"), dict)
        else {}
    )
    ...
    error_codes = sorted(
        {str(item.get("code")) for item in issues if item.get("level") == "error"}
    )
    warning_codes = sorted(
        {str(item.get("code")) for item in issues if item.get("level") == "warning"}
    )
