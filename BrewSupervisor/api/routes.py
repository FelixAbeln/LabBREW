...
def build_router() -> APIRouter:
    ...
    async def validate_scenario_import(
        ...
        return await _proxy_via_agent(
            request, fermenter_id, "scenario_service", "scenario/validate-import"
        )
    ...
    async def import_scenario(
        ...
        return await _proxy_via_agent(
            request, fermenter_id, "scenario_service", "scenario/import"
        )

    @router.get("/fermenters/{fermenter_id}/scenario/repository/download/{filename}")
    def download_scenario_repository_package(
        fermenter_id: str, filename: str, request: Request
    ):
        registry = request.app.state.registry
        proxy = request.app.state.proxy
        node = _get_service_node(registry, fermenter_id, "scenario_service")
        if node is None:
            raise HTTPException(status_code=404, detail="Fermenter not found")
        ...
        response = _read_raw_response(
            ...
            url=_build_service_proxy_url(
                node, "scenario_service", f"scenario/repository/download/{filename}"
            ),
            params=dict(request.query_params),
            stream=True,
        )
    ...
    @router.api_route(
        "/fermenters/{fermenter_id}/scenario/repository/{repo_path:path}",
        methods=["GET", "POST", "PUT", "DELETE"],
    )
    @router.api_route(
        "/fermenters/{fermenter_id}/scenario/repository",
        methods=["GET", "POST", "PUT", "DELETE"],
    )
    async def proxy_scenario_repository(
        fermenter_id: str, request: Request, repo_path: str = ""
    ):
        service_path = (
            f"scenario/repository/{repo_path}".rstrip("/")
            if repo_path
            else "scenario/repository"
        )
        return await _proxy_via_agent(
            request, fermenter_id, "scenario_service", service_path
        )
    ...
    # ------------------------------------------------------------------ scenario run control + queue

    @router.api_route(
        "/fermenters/{fermenter_id}/scenario/run/{run_path:path}",
        methods=["GET", "POST", "PUT", "DELETE"],
    )
    async def proxy_scenario_run(
        fermenter_id: str, request: Request, run_path: str = ""
    ):
        return await _proxy_via_agent(
            request,
            fermenter_id,
            "scenario_service",
            f"scenario/run/{run_path}".rstrip("/"),
        )

    @router.api_route(
        "/fermenters/{fermenter_id}/scenario/queue/{queue_path:path}",
        methods=["GET", "POST", "PUT", "DELETE"],
    )
    @router.api_route(
        "/fermenters/{fermenter_id}/scenario/queue",
        methods=["GET", "POST", "PUT", "DELETE"],
    )
    async def proxy_scenario_queue(
        fermenter_id: str, request: Request, queue_path: str = ""
    ):
        svc_path = (
            f"scenario/queue/{queue_path}".rstrip("/")
            if queue_path
            else "scenario/queue"
        )
        return await _proxy_via_agent(
            request, fermenter_id, "scenario_service", svc_path
        )
