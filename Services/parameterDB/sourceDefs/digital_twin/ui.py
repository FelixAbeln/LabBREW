...
def _build_dynamic_sections(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    input_bindings = (
        dict(config.get("input_bindings") or {})
        if isinstance(config.get("input_bindings"), dict)
        else {}
    )
    output_params = (
        dict(config.get("output_params") or {})
        if isinstance(config.get("output_params"), dict)
        else {}
    )
    summary = {
    ...
    sections = [
        {
            ...
            "fields": [
                {
                    "key": "config.discovered_fmu",
                    "label": "FMU Summary",
                    "type": "json",
                    "readonly": True,
                    "help": (
                        "Auto-generated from the current FMU path when "
                ...
                "fields": [
                    {
                        ...
                        or "Parameter to feed this FMU input.",
                    }
                    for item in inputs
                ],
            }
        ...
        sections.append(
            {
                ...
                "fields": [
                    {
                        "key": "config.input_bindings",
                        "label": "Input Bindings",
                        "type": "json",
                        "required": False,
                        "help": error
                        or "No input variables were discovered from this FMU.",
                    },
                ],
            }
        )
                ...
                "fields": [
                    {
                        ...
                        "help": item.get("description")
                        or (
                            "Optional extra mirror target for this FMU "
                            "output. The twin can also manage its own "
                            "output parameters automatically."
                        ),
                    }
                    for item in outputs
                ],
                ...
                "fields": [
                    {
                        ...
                        "required": False,
                        "help": error
                        or "No output variables were discovered from this FMU.",
                    },
                ],
            }
        )
...
def get_ui_spec(record: dict[str, Any] | None = None, mode: str | None = None) -> dict:
    ...
    edit_sections = [
            ...
            "fields": [
                {
                    ...
                    "required": True,
                    "help": (
                        "Upload or choose an FMU from the datasource FMU "
                        "folder. The edit form discovers inputs and outputs "
        ...
        },
        {
            ...
            "fields": [
                {
                    "key": "config.auto_manage_outputs",
                    "label": "Auto-manage discovered outputs",
                    "type": "bool",
                    "required": False,
                    "help": (
                        "Create one parameter per discovered FMU output "
                        "and remove stale managed outputs when the FMU "
                        "changes."
                    ),
                },
                {
                    "key": "config.managed_output_prefix",
                    "label": "Managed Output Prefix",
                    "type": "string",
                    "required": False,
                    "help": "Defaults to <parameter_prefix>.outputs.",
                },
                {
                    "key": "config.reset_nonce",
                    "label": "Reset Token",
                    "type": "int",
                    "required": False,
                    "help": (
                        "Bump this number and save to force a reinitialize/"
                        "restart of the twin from the admin UI."
                    ),
                },
                {
                    "key": "config.reset_param",
                    "label": "Reset Parameter",
                    "type": "string",
                    "required": False,
                    "help": (
                        "Optional parameter name that can be pulsed/set "
                        "by automation. The twin resets itself when this "
                        "parameter becomes truthy, then clears it back "
                        "to false."
                    ),
                },
            ],
    ...
    ui = {
        ...
        "display_name": "Digital Twin FMU",
        "description": (
            "Creates a digital twin data source from an FMU. Create with "
            "just the FMU path, then edit to bind discovered inputs and "
            "outputs. Discovered outputs can also be auto-created by the "
            "twin itself."
        ),
                    ...
                    "fields": [
                        {
                            ...
                            "required": True,
                            "help": (
                                "Upload or choose an FMU from the "
                                "datasource FMU folder."
                            ),
                        },
                        {
                            "key": "config.enabled",
