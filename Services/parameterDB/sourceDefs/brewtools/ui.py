...
def _transport_section() -> dict:
    return {
        ...
        "fields": [
            ...
            {"key": "config.channel", "label": "Bus / Channel", "type": "int", "required": False, "hint": "Selects which bus this source listens on and sends to. For Kvaser: CAN channel index. For PCAN gateway: bus number in the range 0-255 (encoded as a single byte) — frames from other buses are ignored, and outgoing commands target this bus only. Values outside 0-255 are invalid and may wrap to a different bus (for example, 256 can become 0). Add a second source to use a second bus.", "visible_when": {"config.transport": ["kvaser", "pcan_gateway_udp"]}},
            ...
            {"key": "config.gateway_control_port", "label": "PCAN Gateway Control Port", "type": "int", "required": True, "hint": "TCP control-plane port (typically 45321).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_control_enabled", "label": "Enable Gateway Control Plane", "type": "bool", "required": False, "hint": "Prefilled and recommended ON.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_route_name", "label": "Gateway Route Name", "type": "string", "required": False, "hint": "Prefilled route identifier (normally leave as rt2).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_route_state", "label": "Gateway Route State", "type": "string", "required": False, "hint": "Prefilled route state (normally leave unchanged).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_auth_token", "label": "Gateway Handshake Token", "type": "string", "required": False, "hint": "Developer field. Leave empty for default no-auth behavior.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_auth_id", "label": "Gateway Handshake ID", "type": "string", "required": False, "hint": "Developer field for custom handshake workflows.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_send_fw_dev_probes", "label": "Send FW/DEV Probes", "type": "bool", "required": False, "hint": "Prefilled and recommended ON.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_control_tick_s", "label": "Gateway Control Tick (s)", "type": "float", "required": False, "hint": "Shared keepalive interval for TX/RX routes.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_control_timeout_s", "label": "Gateway Control Timeout (s)", "type": "float", "required": False, "hint": "Shared control-plane timeout for TX/RX.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_rx_control_enabled", "label": "Enable RX Route Hold", "type": "bool", "required": False, "hint": "Maintain RX route on gateway for route parity.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_rx_route_name", "label": "RX Route Name", "type": "string", "required": False, "hint": "Prefilled from capture (rt1).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_rx_route_state", "label": "RX Route State", "type": "string", "required": False, "hint": "Prefilled from capture (0x08000002).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_rx_auth_token", "label": "RX Handshake Token", "type": "string", "required": False, "hint": "Developer field. Leave empty for default no-auth behavior.", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_rx_update_state", "label": "RX Update State", "type": "string", "required": False, "hint": "Route state for keepalive (0xc000002).", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
            {"key": "config.gateway_bind_host", "label": "PCAN Bind Host", "type": "string", "required": False, "hint": "Local bind address (0.0.0.0 for all interfaces)", "visible_when": {"config.transport": "pcan_gateway_udp", "config.gateway_dev_options": "true"}},
        ...
        "module": {
            ...
            "menu": {
                ...
                "result": {
                    ...
                    "error_key": "error",
                    "selected_identity_keys": ["transport", "gateway_host", "interface", "channel"],
