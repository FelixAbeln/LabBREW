...
from ..parameterdb_core.protocol import encode_message, make_error_response, make_response, read_message, validate_request_envelope
    ...
    def handle(self) -> None:
        while True:
            ...
            try:
                ...
                resp = make_error_response(req_id=req_id, error_type=exc.__class__.__name__, message=str(exc))
...
class SourceAdminTCPServer(socketserver.ThreadingTCPServer):
    ...
    def api_ping(self, payload: dict[str, Any]) -> str:
        return 'pong'

    def api_list_source_types_ui(self, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def api_get_source_type_ui(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(name, str) and name.strip():
            try:
                ...
                record = None
        return self.runner.registry.get_ui_spec(source_type, record=record, mode=str(payload.get('mode') or '').strip() or None)

    def api_list_sources(self, payload: dict[str, Any]) -> dict[str, Any]:
