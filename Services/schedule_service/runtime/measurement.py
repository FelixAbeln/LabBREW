...
class _MeasurementMixin:
    ...
    def _prepare_scheduler_sidecar_files(
        ...
        *,
        ...
        schedule_export_path = output_dir_path / f"{session_name}.schedule.json"
        run_log_path = output_dir_path / f"{session_name}.run.log"

        # Export the exact schedule payload that is being executed.
        self._atomic_write_json_file(str(schedule_export_path), schedule.to_dict())
        ...
        bootstrap_lines.extend(
            f"{self._utc_now_iso()} {entry}" for entry in self._status.event_log
        )
        self._atomic_write_text_file(str(run_log_path), "\n".join(bootstrap_lines) + "\n")

        self._schedule_export_path = str(schedule_export_path)
        self._run_log_path = str(run_log_path)

        return [str(schedule_export_path), str(run_log_path)]
    ...
    def _auto_start_measurement_locked(self, schedule: ScheduleDefinition) -> None:
        ...
        If measurement_config contains a 'parameters' list,
        those specific parameters are used.
        ...
        try:
            ...
            self._append_event(
                f"Global measurement start skipped; status unavailable: {exc}"
            )
            return

        if bool(status.get("recording")):
            self._append_event(
                "Global measurement already recording; skipped auto-start"
            )
            return
        ...
        if isinstance(configured_parameters, list) and configured_parameters:
            parameters = [
                str(p).strip() for p in configured_parameters if str(p).strip()
            ]
        else:
        ...
        if not parameters:
            self._append_event(
                "Global measurement skipped: no parameters configured "
                "or available from control snapshot"
            )
            return
    ...
    def _start_global_measurement(
        self, action: Any, _step: ScheduleStep
    ) -> dict[str, Any]:
        ...
        if bool(status.get("recording")):
            return {
                "ok": True,
                "message": "Measurement already recording; skipped setup/start",
            }
        ...
        if isinstance(configured_parameters, list) and configured_parameters:
            parameters = [
                str(item) for item in configured_parameters if str(item).strip()
            ]
        else:
            snapshot = self.control.snapshot()
            values = snapshot.get("values") if isinstance(snapshot, dict) else {}
        ...
        if not parameters:
            raise RuntimeError(
                "global_measurement start requires parameters "
                "or a non-empty control snapshot"
            )
    ...
    def _ensure_measurement_running_locked(self) -> bool:
        ...
        try:
            ...
            self._append_event(
                f"Could not query data service status before loadstep: {exc}"
            )

        schedule = self.repository.get_current()
        if schedule is None:
            self._append_event(
                "Cannot auto-start measurement for loadstep: no schedule loaded"
            )
            return False
        ...
        try:
            ...
            self._append_event(
                f"Could not confirm measurement state after auto-start: {exc}"
            )
            return False

    def _take_data_loadstep(self, action: Any, _step: ScheduleStep) -> dict[str, Any]:
        if not self._ensure_measurement_running_locked():
            raise RuntimeError(
                "take_loadstep requires an active measurement "
                "session, and auto-start failed"
            )
        ...
        if duration in (None, ""):
            ...
            if schedule is not None:
                measurement_cfg = (
                    schedule.measurement_config
                    if isinstance(schedule.measurement_config, dict)
                    else {}
                )
                duration = measurement_cfg.get("loadstep_duration_seconds")
        duration_seconds = float(duration if duration is not None else 30.0)
        loadstep_name = str(
            params.get("loadstep_name") or self._default_data_name(suffix="ls")
        )
        ...
        if isinstance(loadstep_parameters_raw, list):
            parsed = [
                str(item) for item in loadstep_parameters_raw if str(item).strip()
            ]
        ...
        if result.get("ok", False):
            selected_parameters = (
                loadstep_parameters if loadstep_parameters is not None else None
            )
            self._append_data_record(
                {
                    ...
                    "duration_seconds": duration_seconds,
                    "parameters_count": len(selected_parameters)
    ...
    def _run_triggered_loadsteps_locked(
        self, step: ScheduleStep, values: dict[str, Any]
    ) -> None:
        for index, action in enumerate(step.actions):
            ...
            if not loadstep_result.get("ok", False):
                raise RuntimeError(
                    f"Action failed for take_loadstep trigger: {loadstep_result}"
                )
            ...
            if loadstep_name:
                self._append_event(
                    f"Triggered loadstep {loadstep_name} for step {step.name}"
                )
