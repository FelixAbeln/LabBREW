...
class _UtilsMixin:
    ...
    def _collect_values(self, step: ScheduleStep) -> dict[str, Any]:
        ...
        for action in getattr(step, "actions", []) or []:
            if (
                action.kind != "take_loadstep"
                or self._action_timing(action) != "on_trigger"
            ):
                continue
