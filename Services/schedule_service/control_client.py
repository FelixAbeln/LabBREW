...
class ControlClient:
    def __init__(
        self, base_url: str = "http://127.0.0.1:8766", timeout_s: float = 5.0
    ) -> None:
    ...
    def write(self, target: str, value: Any, owner: str) -> dict[str, Any]:
        return self._post(
            "/control/write", {"target": target, "value": value, "owner": owner}
        )

    def ramp(
        self, *, target: str, value: Any, duration_s: float, owner: str
    ) -> dict[str, Any]:
        return self._post(
            "/control/ramp",
            {"target": target, "value": value, "duration": duration_s, "owner": owner},
        )
