...
def split_top_level(text: str, delimiter: str = ";") -> list[str]:
    ...
    for ch in text:
        if ch == "(":
            ...
            if depth < 0:
                raise ValueError(
                    f"Invalid wait syntax '{text}': unmatched closing parenthesis"
                )
...
def parse_condition_expr(expr: str) -> dict[str, Any]:
    ...
    if len(parts) not in {4, 5} or parts[0] != "cond":
        raise ValueError(
            f"Invalid condition syntax '{expr}'. Use "
...
def parse_wait_expr_string(expr: str) -> dict[str, Any] | None:
    ...
    if expr.startswith("all(") and expr.endswith(")"):
        ...
        if not parts:
            raise ValueError(
                "Invalid wait syntax 'all()' or empty all(...); "
                "at least one child expression is required"
            )
        ...
        for part in parts:
            ...
            if child is None:
                raise ValueError(
                    "Invalid wait syntax in all(...); empty child "
    ...
    if expr.startswith("any(") and expr.endswith(")"):
        ...
        if not parts:
            raise ValueError(
                "Invalid wait syntax 'any()' or empty any(...); "
                "at least one child expression is required"
            )
        children: list[dict[str, Any]] = []
        for part in parts:
            ...
            if child is None:
                raise ValueError(
                    "Invalid wait syntax in any(...); empty child "
                    "expressions are not allowed"
                )
            children.append(child)
        return {
    ...
    if expr.startswith("rising(") and expr.endswith(")"):
        ...
        if child is None:
            raise ValueError(
                "Invalid wait syntax in rising(...); child expression is required"
            )
        return {
            "kind": "rising",
            "child": child,
    ...
    if expr.startswith("falling(") and expr.endswith(")"):
        ...
        if child is None:
            raise ValueError(
                "Invalid wait syntax in falling(...); child expression is required"
            )
        return {
            "kind": "falling",
            "child": child,
    ...
    if expr.startswith("pulse(") and expr.endswith(")"):
        ...
        if len(parts) != 2:
            raise ValueError(
                "Invalid wait syntax for pulse(...). Use pulse(expr;hold_seconds)"
            )
        child = parse_wait_expr_string(parts[0])
        if child is None:
            raise ValueError(
                "Invalid wait syntax in pulse(...); child expression is required"
            )
        try:
    ...
    raise ValueError(
        f"Invalid wait syntax '{expr}'. Use elapsed:..., cond:..., "
        "all(...), any(...), rising(...), falling(...), or pulse(...;seconds)"
    )
