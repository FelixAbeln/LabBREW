...
from ..operator_engine import AtomicCondition, CompositeCondition, ConditionEngine, EvaluationState
...
class WaitEngine:
    ...
    def _evaluate(self, spec: WaitSpec, *, context: WaitContext, state: WaitState, path: str) -> WaitResult:
        if spec.kind == 'none':
        ...
        if spec.kind in {'rising', 'falling', 'pulse'}:
            ...
            if spec.kind == 'rising':
                ...
                if node_state.pulse_started_monotonic is None:
                    ...
                    elapsed_since_pulse = max(0.0, context.now_monotonic - node_state.pulse_started_monotonic)
            ...
            if spec.kind == 'pulse':
                ...
                if node_state.pulse_started_monotonic is None:
                    ...
                    elapsed_since_pulse = max(0.0, context.now_monotonic - node_state.pulse_started_monotonic)
                message = (
                    f'pulse active {elapsed_since_pulse:.2f}/{hold_s:.2f}s' if edge_detected
                ...
                message = f'{spec.kind} edge' if edge_detected else f'waiting for {spec.kind} edge'
                observed = dict(child_result.observed_values)
        ...
        if spec.kind in {"all_of", "any_of"}:
            ...
            child_messages = [c.message.strip() for c in children if c.message and c.message.strip()]
...
def parse_wait_spec(payload: Any) -> WaitSpec | None:
    ...
    if kind in {'all_of', 'any_of'}:
        ...
        children = tuple(parse_wait_spec(child) or WaitSpec(kind='none') for child in children_payload)
        return WaitSpec(
            kind=kind,
            children=children,
            label=str(payload.get('label', '') or ''),
...
def parse_condition_node(payload: Any) -> AtomicCondition | CompositeCondition:
    ...
    if 'all' in payload:
        children = tuple(parse_condition_node(item) for item in payload.get('all') or [])
        return CompositeCondition(kind='all', children=children, for_s=float(payload.get('for_s') or 0.0))
    if 'any' in payload:
        children = tuple(parse_condition_node(item) for item in payload.get('any') or [])
        return CompositeCondition(kind='any', children=children, for_s=float(payload.get('for_s') or 0.0))
    if 'not' in payload:
        child = parse_condition_node(payload['not'])
        return CompositeCondition(kind='not', children=(child,), for_s=float(payload.get('for_s') or 0.0))
