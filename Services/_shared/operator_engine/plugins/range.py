...
PLUGINS = [
    CallableOperator(
        ...
        fn=lambda value, params: as_float(params['min']) <= as_float(value) <= as_float(params['max']),
    ),
    CallableOperator(
        metadata=OperatorMetadata(
            name='out_of_range',
        ...
        fn=lambda value, params: not (as_float(params['min']) <= as_float(value) <= as_float(params['max'])),
    ),
]
