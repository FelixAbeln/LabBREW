...
class CliRenderer:
    def render_args(self, service: ServiceSpec, bindings: dict[str, CapabilityBinding]) -> list[str]:
        args = list(service.static_args)
        rules_by_capability = {rule.capability: rule for rule in service.capability_arg_rules}

        for capability in service.requires:
            rule = rules_by_capability.get(capability)
            if rule is None:
                raise ResolutionError(
                    f"Service '{service.name}' requires capability '{capability}' but has no arg rule"
                )
            ...
            if rule.mode == "url":
                ...
                args.extend([rule.host_flag, endpoint.host, rule.port_flag, str(endpoint.port)])
            else:
